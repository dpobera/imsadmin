...
const selectItem = function (e) {
    ...
    // const itemPrice = targetItem.querySelector(".cost").innerHTML;

    // Check for duplicate entries
    if (hasDuplicate(+itemCode, tableItemTb))
        return alert(`${itemName} is already added.`);

    const joQty = prompt("Enter Order Quantity");
    const joPrice = prompt("Enter Price: ");
    const itemDiscPercent = 0;
    const itemDiscVal = 0;
    const totalCost = joQty * joPrice;
    ...
    <td class='td__edit td__edit--cost'>${formatNumber(joPrice)}</td> 
    ...
    <input type='hidden' name='itemPrice[]' value='${joPrice}' class='input__edit input__edit--cost'>
